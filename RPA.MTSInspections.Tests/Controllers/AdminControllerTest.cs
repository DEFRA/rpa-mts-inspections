...
using PagedList;
using RPA.MTSInspections.Controllers;
...
namespace RPA.MTSInspections.Tests.Controllers
{
    [TestFixture]
    class AdminControllerTest
    {
        ...
        public void Setup()
        {
            ...
            plantData = new List<Plant>
            {
               plant1,plant2,plant3

            };

            var audit1 = new Audit { Id = 1, PlantId = Guid.Parse("A462AEEE-537A-4E5D-802D-9B2D6F6F979D"), Date = new DateTime(2018, 1, 1), User = "Test User", Action = "Plant", PropertyName = "PlantName", Value = "Beefy Mc Beef Plant2", NewValue = "Beefy Mc Beef Plant4" };

            auditData = new List<Audit>
            {
                audit1
            };
            ...
            riskScoreData = new List<RiskScore>
            {
            ...
            criteriaData = new List<Criteria>
            {
            ...
            plantOptionData = new List<PlantOption>
            {
            ...
            lastInspectionData = new List<LastInspection>
            {
            ...
            plant1.LastInspection = lastInspectionData.ToList();
            plant2.LastInspection = lastInspectionData.ToList();
            plant3.LastInspection = lastInspectionData.ToList();

            //Create mocks for teams repository and setup mocked behaviour to mimic EF using test data above
            mockPlantSet = new Mock<DbSet<Plant>>().SetupData(plantData);
            mockPlantSet.Setup(x => x.AsNoTracking()).Returns(mockPlantSet.Object);
            mockPlantSet.Setup(x => x.Find(It.IsAny<object>())).Returns(plantData.FirstOrDefault());
            mockPlantSet.Setup(x => x.Include(It.IsAny<string>())).Returns(mockPlantSet.Object);

            mockSchemeSet = new Mock<DbSet<Scheme>>().SetupData(schemeData);
            mockSchemeSet.Setup(x => x.Find(It.IsAny<object>())).Returns(schemeData.FirstOrDefault());
            mockSchemeSet.Setup(x => x.AsNoTracking()).Returns(mockSchemeSet.Object);
            mockSchemeSet.Setup(x => x.Include(It.IsAny<string>())).Returns(mockSchemeSet.Object);

            mockCriteriaSet = new Mock<DbSet<Criteria>>().SetupData(criteriaData);
            mockCriteriaSet.Setup(x => x.Find(It.IsAny<object>())).Returns(criteriaData.FirstOrDefault());
            mockCriteriaSet.Setup(x => x.AsNoTracking()).Returns(mockCriteriaSet.Object);
            mockCriteriaSet.Setup(x => x.Include(It.IsAny<string>())).Returns(mockCriteriaSet.Object);

            mockOptionSet = new Mock<DbSet<Option>>().SetupData(optionData);
            mockOptionSet.Setup(x => x.Find(It.IsAny<object>())).Returns(optionData.FirstOrDefault());
            mockOptionSet.Setup(x => x.AsNoTracking()).Returns(mockOptionSet.Object);
            mockOptionSet.Setup(x => x.Include(It.IsAny<string>())).Returns(mockOptionSet.Object);

            mockPlantOptionSet = new Mock<DbSet<PlantOption>>().SetupData(plantOptionData);
            mockPlantOptionSet.Setup(x => x.Find(It.IsAny<object>())).Returns(plantOptionData.FirstOrDefault());
            mockPlantOptionSet.Setup(x => x.AsNoTracking()).Returns(mockPlantOptionSet.Object);
            mockPlantOptionSet.Setup(x => x.Include(It.IsAny<string>())).Returns(mockPlantOptionSet.Object);

            mockRiskScoreSet = new Mock<DbSet<RiskScore>>().SetupData(riskScoreData);
            mockRiskScoreSet.Setup(x => x.Find(It.IsAny<object>())).Returns(riskScoreData.FirstOrDefault());
            mockRiskScoreSet.Setup(x => x.AsNoTracking()).Returns(mockRiskScoreSet.Object);
            mockRiskScoreSet.Setup(x => x.Include(It.IsAny<string>())).Returns(mockRiskScoreSet.Object);

            mockLastInspectionSet = new Mock<DbSet<LastInspection>>().SetupData(lastInspectionData);
            mockLastInspectionSet.Setup(x => x.Find(It.IsAny<object>())).Returns(lastInspectionData.FirstOrDefault());
            mockLastInspectionSet.Setup(x => x.AsNoTracking()).Returns(mockLastInspectionSet.Object);
            mockLastInspectionSet.Setup(x => x.Include(It.IsAny<string>())).Returns(mockLastInspectionSet.Object);

            mockAuditSet = new Mock<DbSet<Audit>>().SetupData(auditData);
            mockAuditSet.Setup(x => x.Find(It.IsAny<object>())).Returns(auditData.FirstOrDefault());
            mockAuditSet.Setup(x => x.AsNoTracking()).Returns(mockAuditSet.Object);
            mockAuditSet.Setup(x => x.Include(It.IsAny<string>())).Returns(mockAuditSet.Object);
            ...
            mockContext.Setup(x => x.RiskScore).Returns(mockRiskScoreSet.Object);
            mockContext.Setup(x => x.Audit).Returns(mockAuditSet.Object);

            //Setup Service Injecting Mocked Context
            Pservice = new PlantService(mockContext.Object);
            Aservice = new AuditService(mockContext.Object);
            Cservice = new CriteriaService(mockContext.Object);

            //Setup controller
            controller = new AdminController(mockContext.Object, Cservice, Pservice, Aservice);
        }

        //GET
        [Test]
        public void Test_BlsOptions()
        {
            var result = controller.BlsOptions() as ViewResult;

            Assert.AreEqual(11, ((List<Criteria>)result.ViewData.Model).Count);
        }

        [Test]
        public void Test_BccOptions()
        {
            var result = controller.BccOptions() as ViewResult;

            Assert.AreEqual(15, ((List<Criteria>)result.ViewData.Model).Count);
        }

        [Test]
        public void Test_PcgOptions()
        {
            var result = controller.PcgOptions() as ViewResult;

            Assert.AreEqual(14, ((List<Criteria>)result.ViewData.Model).Count);
        }

        //POST
        [Test]
        public void Test_BlsEdit()
        {
            var schemeBLS = new Scheme { SchemeId = Guid.Parse("EE9597C9-9479-4486-8506-1374D7F62A16"), Name = "BLS", Weighting = 0, Active = true };
            var criteria1 = new Criteria { CriteriaId = Guid.Parse("C65CC653-C9B6-494F-9847-F7DEA02DDD43"), Name = "Number of Non-Compliance", SchemeId = Guid.Parse("EE9597C9-9479-4486-8506-1374D7F62A16"), Weighting = 10, Scheme = schemeBLS };
            var option1 = new Option { OptionId = Guid.Parse("3C79CCB1-FE98-48C6-8F9E-072B7B97DE6D"), Name = "2>3", CriteriaId = Guid.Parse("C65CC653-C9B6-494F-9847-F7DEA02DDD43"), Active = true, Score = 4, Criteria = criteria1 };

            var result = controller.BlsEdit(option1) as ViewResult;

            mockContext.Verify(x => x.SaveChanges(), Times.Once);
        }

        [Test]
        public void Test_BccEdit()
        {
            var schemeBCC = new Scheme { SchemeId = Guid.Parse("5982F25B-4DDE-483C-B0D3-F2E46A5036B4"), Name = "BCC", Weighting = 0, Active = true };
            var criteria12 = new Criteria { CriteriaId = Guid.Parse("9B225621-875E-4324-B341-1D6FC8CB5402"), Name = "Number of Near Misses in Previous 3 Years", SchemeId = Guid.Parse("5982F25B-4DDE-483C-B0D3-F2E46A5036B4"), Weighting = 20, Scheme = schemeBCC };
            var option12 = new Option { OptionId = Guid.Parse("3602988E-497D-4BB0-80AE-18791916C842"), Name = "4", CriteriaId = Guid.Parse("9B225621-875E-4324-B341-1D6FC8CB5402"), Active = true, Score = 8, Criteria = criteria12 };

            var result = controller.BccEdit(option12) as ViewResult;

            mockContext.Verify(x => x.SaveChanges(), Times.Once);
        }

        [Test]
        public void Test_PcgEdit()
        {
            var schemePCG = new Scheme { SchemeId = Guid.Parse("F5C4F862-9798-4F5C-B64E-CB6FAD81B0EF"), Name = "PCG", Weighting = 0, Active = true };
            var criteria27 = new Criteria { CriteriaId = Guid.Parse("70C39842-B3E2-4C99-8D04-09050102C723"), Name = "Record Keeping", SchemeId = Guid.Parse("F5C4F862-9798-4F5C-B64E-CB6FAD81B0EF"), Weighting = 7, Scheme = schemePCG };
            var option27 = new Option { OptionId = Guid.Parse("35705372-538F-4832-BF61-6299C93C324F"), Name = "Satisfactory", CriteriaId = Guid.Parse("70C39842-B3E2-4C99-8D04-09050102C723"), Active = true, Score = 10, Criteria = criteria27 };

            var result = controller.PcgEdit(option27) as ViewResult;

            mockContext.Verify(x => x.SaveChanges(), Times.Once);
        }

        public void Test_BlsCreate()
        {
            var schemeBLS = new Scheme { SchemeId = Guid.Parse("EE9597C9-9479-4486-8506-1374D7F62A16"), Name = "BLS", Weighting = 0, Active = true };
            var criteria1 = new Criteria { CriteriaId = Guid.Parse("C65CC653-C9B6-494F-9847-F7DEA02DDD43"), Name = "Number of Non-Compliance", SchemeId = Guid.Parse("EE9597C9-9479-4486-8506-1374D7F62A16"), Weighting = 10, Scheme = schemeBLS };
            var option1 = new Option { OptionId = Guid.Parse("3C79CCB1-FE98-48C6-8F9E-072B7B97DE6D"), Name = "2>3", CriteriaId = Guid.Parse("C65CC653-C9B6-494F-9847-F7DEA02DDD43"), Active = true, Score = 4, Criteria = criteria1 };

            var result = controller.BlsCreate(option1) as ViewResult;

            mockContext.Verify(x => x.SaveChanges(), Times.Once);
        }

        [Test]
        public void Test_BccCreate()
        {
            var schemeBCC = new Scheme { SchemeId = Guid.Parse("5982F25B-4DDE-483C-B0D3-F2E46A5036B4"), Name = "BCC", Weighting = 0, Active = true };
            var criteria12 = new Criteria { CriteriaId = Guid.Parse("9B225621-875E-4324-B341-1D6FC8CB5402"), Name = "Number of Near Misses in Previous 3 Years", SchemeId = Guid.Parse("5982F25B-4DDE-483C-B0D3-F2E46A5036B4"), Weighting = 20, Scheme = schemeBCC };
            var option12 = new Option { OptionId = Guid.Parse("3602988E-497D-4BB0-80AE-18791916C842"), Name = "4", CriteriaId = Guid.Parse("9B225621-875E-4324-B341-1D6FC8CB5402"), Active = true, Score = 8, Criteria = criteria12 };

            var result = controller.BccCreate(option12) as ViewResult;

            mockContext.Verify(x => x.SaveChanges(), Times.Once);
        }

        [Test]
        public void Test_PcgCreate()
        {
            var schemePCG = new Scheme { SchemeId = Guid.Parse("F5C4F862-9798-4F5C-B64E-CB6FAD81B0EF"), Name = "PCG", Weighting = 0, Active = true };
            var criteria27 = new Criteria { CriteriaId = Guid.Parse("70C39842-B3E2-4C99-8D04-09050102C723"), Name = "Record Keeping", SchemeId = Guid.Parse("F5C4F862-9798-4F5C-B64E-CB6FAD81B0EF"), Weighting = 7, Scheme = schemePCG };
            var option27 = new Option { OptionId = Guid.Parse("35705372-538F-4832-BF61-6299C93C324F"), Name = "Satisfactory", CriteriaId = Guid.Parse("70C39842-B3E2-4C99-8D04-09050102C723"), Active = true, Score = 10, Criteria = criteria27 };

            var result = controller.PcgCreate(option27) as ViewResult;

            mockContext.Verify(x => x.SaveChanges(), Times.Once);
        }

        public void Test_OptionPartial()
        {
            var schemeBLS = new Scheme { SchemeId = Guid.Parse("EE9597C9-9479-4486-8506-1374D7F62A16"), Name = "BLS", Weighting = 0, Active = true };
            var criteria1 = new Criteria { CriteriaId = Guid.Parse("C65CC653-C9B6-494F-9847-F7DEA02DDD43"), Name = "Number of Non-Compliance", SchemeId = Guid.Parse("EE9597C9-9479-4486-8506-1374D7F62A16"), Weighting = 10, Scheme = schemeBLS };
            var option1 = new Option { OptionId = Guid.Parse("3C79CCB1-FE98-48C6-8F9E-072B7B97DE6D"), Name = "2>3", CriteriaId = Guid.Parse("C65CC653-C9B6-494F-9847-F7DEA02DDD43"), Active = true, Score = 4, Criteria = criteria1 };

            var result = controller._OptionPartial(Guid.Parse("C65CC653-C9B6-494F-9847-F7DEA02DDD43")) as PartialViewResult;

            Assert.AreEqual("2>3", ((Option)result.ViewData.Model).Name);
        }

        [Test]
        public void Test_WeightingScore()
        {
            var schemeBLS = new Scheme { SchemeId = Guid.Parse("EE9597C9-9479-4486-8506-1374D7F62A16"), Name = "BLS", Weighting = 0, Active = true };
            var criteria1 = new Criteria { CriteriaId = Guid.Parse("C65CC653-C9B6-494F-9847-F7DEA02DDD43"), Name = "Number of Non-Compliance", SchemeId = Guid.Parse("EE9597C9-9479-4486-8506-1374D7F62A16"), Weighting = 10, Scheme = schemeBLS };
            var option1 = new Option { OptionId = Guid.Parse("3C79CCB1-FE98-48C6-8F9E-072B7B97DE6D"), Name = "2>3", CriteriaId = Guid.Parse("C65CC653-C9B6-494F-9847-F7DEA02DDD43"), Active = true, Score = 4, Criteria = criteria1 };

            var result = controller._WeightingScore(Guid.Parse("C65CC653-C9B6-494F-9847-F7DEA02DDD43")) as PartialViewResult;

            Assert.AreEqual("Number of Non-Compliance", ((Criteria)result.ViewData.Model).Name);
        }

        [Test]
        public void Test_EditWeightingScore()
        {
            var schemeBLS = new Scheme { SchemeId = Guid.Parse("EE9597C9-9479-4486-8506-1374D7F62A16"), Name = "BLS", Weighting = 0, Active = true };
            var criteria1 = new Criteria { CriteriaId = Guid.Parse("C65CC653-C9B6-494F-9847-F7DEA02DDD43"), Name = "Number of Non-Compliance", SchemeId = Guid.Parse("EE9597C9-9479-4486-8506-1374D7F62A16"), Weighting = 10, Scheme = schemeBLS };
            var option1 = new Option { OptionId = Guid.Parse("3C79CCB1-FE98-48C6-8F9E-072B7B97DE6D"), Name = "2>3", CriteriaId = Guid.Parse("C65CC653-C9B6-494F-9847-F7DEA02DDD43"), Active = true, Score = 4, Criteria = criteria1 };
            ...
            var result = controller.EditWeightingScore(criteria1) as PartialViewResult;
        ...
        //Plant
        [Test]
        public void Test_Plants()
        {
            var result = controller.Plants(null, 1, 15) as ViewResult;

            Assert.AreEqual(3, ((PagedList<Plant>)result.ViewData.Model).Count);
        }
        ...
        public void Test_Create()
        {
            ...
            var result = controller.Create(plant2) as ViewResult;
        ...
        public void Test_Edit()
        {
            ...
            var result = controller.Edit(plant2, "Edit") as ViewResult;

            mockContext.Verify(x => x.SaveChanges(), Times.Once);
        }

        [Test]
        public void Test_PlantAudit()
        {
            var plant2 = new Plant { PlantId = Guid.Parse("A462AEEE-537A-4E5D-802D-9B2D6F6F979D"), PlantName = "Beefy Mc Beef Plant2", AddressOne = "Beef Street", AddressTwo = "Beef Estate", TownCity = "Beef Town", PostCode = "BEE FY1", County = "BeefHampton", Active = true, LicenceNo = "4445", BLS = true };

            var result = controller.PlantAudit("Beefy Mc Beef Plant2", 1, 15) as ViewResult;

            Assert.AreEqual(1, ((PagedList<Plant>)result.ViewData.Model).Count);
        }

        [Test]
        public void Test_AuditView()
        {
            var audit1 = new Audit { Id = 1, PlantId = Guid.Parse("A462AEEE-537A-4E5D-802D-9B2D6F6F979D"), Date = new DateTime(2018, 1, 1), User = "Test User", Action = "Plant", PropertyName = "PlantName", Value = "Beefy Mc Beef Plant2", NewValue = "Beefy Mc Beef Plant4" };
            var plant2 = new Plant { PlantId = Guid.Parse("A462AEEE-537A-4E5D-802D-9B2D6F6F979D"), PlantName = "Beefy Mc Beef Plant2", AddressOne = "Beef Street", AddressTwo = "Beef Estate", TownCity = "Beef Town", PostCode = "BEE FY1", County = "BeefHampton", Active = true, LicenceNo = "4445", BLS = true };

            var result = controller.AuditView(Guid.Parse("A462AEEE-537A-4E5D-802D-9B2D6F6F979D"), null, 1, 15) as ViewResult;

            Assert.AreEqual(1, ((PagedList<Audit>)result.ViewData.Model).Count);
        }
